...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        评估函数接受当前和建议的后继GameStates（pacman.py）并返回一个数字，数字越大越好。
        
        The evaluation function takes in the current and proposed successor
        GameStates (pacman.py) and returns a number, where higher numbers are better.

        下面的代码从状态中提取一些有用的信息，例如剩余食物（newFood）和移动后吃豆子的位置（newPos）。
        newScaredTimes保留了每个幽灵由于吃豆人吃掉能量颗粒而仍会害怕的剩余时间。
        ...
        food_scores = [util.manhattanDistance(newPos, food) for food in oldFood.asList()]
        # 得到在下一个位置能得到的最大分数
        bestScore = 0
        if len(food_scores) != 0:
            bestScore = min(food_scores)
        #print(food_scores,max(food_scores))
        if oldFood[newPos[0]][newPos[1]] == True: x = 20
        else: x= 0
        ##计算鬼的权重，离鬼越近减分越多
        gost_scores = [util.manhattanDistance(newPos, ghostState.getPosition()) for ghostState in newGhostStates]
        min_ghost_dist = min(gost_scores)
        #print(gost_scores,sum(gost_scores))
        import math
        #print(min_ghost_dist)
        return  x - bestScore * 1.5 - math.exp(-(min_ghost_dist-5))
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        #print('访问深度',self.depth)
        def AlphaBeta(state):
            alpha = float('-Inf') #最大值
            beta = float('Inf')   #最小值
                        
            return MAX(state,0,alpha,beta)
        ...
        def MAX(state,depth,alpha,beta):
            ...
            if depth >= self.depth+1:
                #print("深度",depth,self.evaluationFunction(state))
                return self.evaluationFunction(state)
            ...
            for action in actions:
                v = max(v,MINI(state.generateSuccessor(0,action),depth,1,alpha,beta))
                #print("MAX",action,"v",v,'depth',depth,'alpha',alpha,'beta',beta)
                if v > beta:
                    if depth == 1:
                        return action
                    else:
            ...
            if len(actions) == 0:
                #print('MAX 空扩展',self.evaluationFunction(state))
                return self.evaluationFunction(state)
        ...
        ## MINI函数    
        def MINI(state,depth,depth_ghost,alpha,beta):
            #print("MINI")
            # 到达最深的时候
            ...
            for action in actions:
                if depth_ghost != state.getNumAgents()-1:
                    v = min(v,MINI(state.generateSuccessor(depth_ghost,action),depth,depth_ghost+1,alpha,beta))
                else:
                    v = min(v,MAX(state.generateSuccessor(depth_ghost,action),depth,alpha,beta))
                #print("MIN",action,"v",v,'depth',depth,'alpha',alpha,'beta',beta)
                if v < alpha:
                    return v
                else:
                    beta = min(beta,v)
            if len(actions) == 0:
                #print('MINI 空扩展',self.evaluationFunction(state))
                return self.evaluationFunction(state)
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def MINI(state,depth,depth_ghost):
            ...
            # 只要修改这里就能达到Expectimax了
            #return sum(MINI_list)/len(actions)
            return min(MINI_list)
        ...
        def MAX(state,depth):
            ...
            return sum(MAX_list)/len(actions) 
            #return max(MAX_list)
...
def betterEvaluationFunction(currentGameState):
    ...
    gostPos = [ghostState.getPosition() for ghostState in ghostStates]
